...
function reporeset() {
  ...
  cd .repo/manifests && git reset --hard m/XOS-12.1
...
function reposterilize() {
  ...
  while read dir; do
    ...
    git reset --hard XOS/XOS-12.1 2>/dev/null \
 || git reset --hard github/XOS-12.1 2>/dev/null \
...
function resetmanifest() {
  ...
  git fetch origin XOS-12.1 2>&1 >/dev/null
  git reset --hard origin/XOS-12.1 2>&1 >/dev/null
  cd $(gettop)
}
