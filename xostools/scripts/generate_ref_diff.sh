...
for path in $all_paths; do
    ...
    gen_dest="$GEN_DIR/$FROM_REF/$TO_REF/$path"
    genfile_diff="$gen_dest/diff.md"
    genfile_summary="$gen_dest/summary.md"
    ...
    for commit in $(git log --pretty=format:%h --no-merges $from_commit_hash..$to_commit_hash); do
        ...
        echo "$short_hash: $subject ($author_email)" >> "$genfile_summary"
    done
